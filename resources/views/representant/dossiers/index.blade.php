        ...
        <div class="form-group breadcrumb-right">
            <div class="dropdown">
                ...
                <div class="dropdown-menu dropdown-menu-right">
                    <a class="dropdown-item" href="{{url('representant/dossiers/create')}}"><i class="mr-1" data-feather="plus"></i><span class="align-middle">Nouveau dossier</span></a>
                </div>
            </div>
        </div>
    </div>
                            ...
                            <tr>
                                <td>
                                    @if($dossier->image!==null)
                                        <img src=" {{asset('uploads/dossier/'.$dossier->image)}}" class="mr-75" height="40" width="40">
                                    @else
                                    <img src="{{asset('uploads/dossier/user.png')}}" class="mr-75" height="40" width="40" alt="" />
                                    @endif
                                    <a href="{{url('representant/dossiers/show/'.$dossier->id)}}" class="btn btn-primary">ID:{{$dossier->id}}</a>
                                </td>
                                <td>{{$dossier->prenom}} {{$dossier->nom}}</td>
                                <td>{{$dossier->convention}}</td>
                                <td><a href="{{ url('representant/dossiers/'.$dossier->id.'/effetsmarquants')}}" class="nav-link"><i data-feather='archive'></i></a></td>
                                <td>
                                    <a href="{{url('/representant/forumMedPatient/createbyid/'.$dossier->id)}}" class="nav-link">
                                ...
                                <td>
                                    <div class="dropdown">
                                        ...
                                        <div class="dropdown-menu">
                                            <a href="{{ url('representant/dossiers/'.$dossier->id.'/historiques')}}" class="dropdown-item">
                                                <i data-feather="clock" class="mr-50"></i>Historiques
                                            </a>
                                            <a class="dropdown-item" href="{{url('representant/dossiers/edit/'.$dossier->id)}}">
