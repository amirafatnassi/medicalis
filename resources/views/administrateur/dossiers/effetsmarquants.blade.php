...
<div class="row">
    ...
    <div class="col-3"><a href="{{url('administrateur/dossiers/'.$dossier->id.'/listeSupprimer')}}" class="btn btn-primary">Liste des antécédants Supprimés</a>&nbsp;</div>
    <div class="col-6">
    ...
    <div class="col-12">
        <table class="table" id="myTable">
            ...
            <tbody>
                ...
                <tr>
                    <td><a href="{{url('administrateur/'.$dossier->id.'/consultation/'.$consultation->id.'/show')}}" class="btn btn-primary">ID:{{$consultation->id}}</a></td>
                    <td>{{date('d/m/Y',strtotime($consultation->date))}}</td>
                    <td>{{$consultation->effet_marquant_txt}}</td>
                    <td>
                        @if(!is_null($consultation->downloads))
                        <a href="{{url('administrateur/'.$id_dossier.'/consultation/showExamenfiles/'.$consultation->id)}}"><i data-feather="paperclip"></i></a>
                        @endif
                    </td>
...
<script>
    ...
    function deleteData(id) {
        var id = id;
        var url = '{{route("administrateur.dossiers.deleteHistorique",":id")}}';
