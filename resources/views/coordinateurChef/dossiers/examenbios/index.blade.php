...
<div class="row">
    ...
    <div class="col-2">
        <a href="{{url('coordinateurChef/dossiers/'.$dossier->id.'/examenbios/create')}}" class="btn btn-outline-success">Nouveau examen bio</a>
    </div>
    <div class="col-6">
    ...
    <div class="col-12">
        <table class="table" id="myTable">
            ...
            <tbody>
                ...
                <tr>
                    <td><a href="{{url('coordinateurChef/dossiers/examenbios/show/'.$examenbio->id)}}" class="btn btn-primary">ID:{{$examenbio->id}}</a></td>
                    <td>{{date('d/m/Y',strtotime($examenbio->date))}}</td>
                    <td>{{$examenbio->medecin}}</td>
                    <td>{{$examenbio->specialite}}</td>
                    <td>
                        @if(($examenbio->downloads)!==0)
                        <a href="{{url('coordinateurChef/dossiers/examenbios/showExamenfiles/'.$examenbio->id)}}"><i data-feather="paperclip"></i></a>
                        @endif
                    </td>
