...
<div class="row">
    <div class="col-3">
        <h3>Liste de représentants sur la platforme:</h3>
    </div>
    <div class="col-3">
...
<table id="myTable" class="table table-striped">
    ...
    @foreach($mesRepresentants as $c)
    <tr class="list-goup-item">
        <td><a type="button" class="btn btn-outline-primary" href="{{url('coordinateurChef/representants/show',$c->id)}}"> {{$c -> id}}</a></td>
    ...
    <tr class="list-goup-item">
        <td><a type="button" class="btn btn-outline-primary" href="{{url('coordinateurChef/representants/show',$rep->id)}}"> {{$rep -> id}}</a></td>
        <td>{{$rep->prenom}} {{$rep->nom}}</td>
        <td>{{$rep->role}}</td>
        <td>{{$rep->tel}}</td>
        <td>{{$rep->email}}</td>
        <td>
            <form method="POST" action="{{url('coordinateurChef/representants/activer/'.$rep->repcoord)}}" enctype="multipart/form-data"> {{csrf_field()}}
    ...
    @foreach($others as $o)
    <tr class="list-goup-item">
        <td><a type="button" class="btn btn-outline-primary" href="{{url('coordinateurChef/representants/show',$o->id)}}"> {{$o -> id}}</a></td>
        <td>{{$o->prenom}} {{$o->nom}}</td>
        <td>{{$o->role}}</td>
        <td>{{$o->tel}}</td>
        <td>{{$o->email}}</td>
        <td>
            <form method="POST" action="{{url('coordinateurChef/representants/ajouter/'.$o->id)}}" enctype="multipart/form-data"> {{csrf_field()}}
...
<script>
    ...
    function deleteData(id) {
        var id = id;
        var url = '{{route("coordinateurChef.representants.supprimer",":id")}}';
